...
export function validateBuild(
  ...
  if (budgetRemaining > maxBudget * 0.3) {
    warnings.push(
      `Large budget remaining (${budgetRemaining} Stadium Cash). Consider adding more items.`
    );
  }
...
export function replaceItemInBuild(
  build: PlayerBuild,
  oldItemId: string,
  newItem: StadiumItem
): PlayerBuild {
  const newItems = build.currentItems.map(item => (item.id === oldItemId ? newItem : item));
  return updateBuild(build, newItems);
}
    ...
    for (const [statKey, target] of Object.entries(targetStats)) {
      if (target && item.stats[statKey as keyof StadiumStats]) {
        ...
        const priorityMultiplier =
          target.priority === 'high' ? 3 : target.priority === 'medium' ? 2 : 1;
