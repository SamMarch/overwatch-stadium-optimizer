...
export function optimizeItemBuild(criteria: OptimizationCriteria): OptimizationResult {
  ...
  try {
    ...
    const alternativeBuild = findAlternativeBuild(
      availableItems,
      knapsackResult,
      criteria
    );
...
function solve3DKnapsack(
  ...
  const dp: number[][][] = Array(n + 1).fill(null).map(() =>
    Array(maxBudget + 1).fill(null).map(() =>
...
function findItemsToSell(
  currentItems: StadiumItem[],
  targetItems: StadiumItem[]
): StadiumItem[] {
...
function explainOptimization(
  ...
  if (categoryNames.length > 1) {
    explanations.push(`Balanced build with ${categoryNames.join(', ')} items for comprehensive coverage.`);
  }
  ...
  if (passiveItems.length > 0) {
    explanations.push(`Includes ${passiveItems.length} items with special passive effects for tactical advantage.`);
  }
  
  return explanations;
}

function validateOptimizationCriteria(criteria: OptimizationCriteria): { isValid: boolean; errors: string[] } {
  const errors: string[] = [];
  ...
  const hasTargetStats = Object.values(criteria.targetStats).some(target => 
...
function createFallbackResult(criteria: OptimizationCriteria): OptimizationResult {
  ...
  const fallbackItems = STADIUM_ITEMS
    .filter(item => item.cost <= criteria.maxBudget && item.rarity === 'common')
